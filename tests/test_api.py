...
class ApiHttpValidationTests(unittest.TestCase):
    ...
    def test_product_crud_and_offer_delete_endpoints(self) -> None:
        status, product = self._request("POST", "/api/products", {
            "title": "ThinkPad X1", "brand": "Lenovo", "category": "laptops", "condition": "used"
        })
        self.assertTrue(status.startswith("201"))

        status, fetched = self._request("GET", f"/api/products/{product['id']}")
        self.assertTrue(status.startswith("200"))
        self.assertEqual(fetched["title"], "ThinkPad X1")

        status, updated = self._request("PUT", f"/api/products/{product['id']}", {
            "title": "ThinkPad X1 Carbon", "brand": "Lenovo", "category": "laptops", "condition": "used"
        })
        self.assertTrue(status.startswith("200"))
        self.assertEqual(updated["title"], "ThinkPad X1 Carbon")

        status, offer = self._request("POST", "/api/offers", {
            "product_id": product["id"],
            "seller": {"name": "OfficeStore", "city": "Baku"},
            "price_azn": 2100,
            "currency": "AZN"
        })
        self.assertTrue(status.startswith("201"))

        status, deleted_offer = self._request("DELETE", f"/api/offers/{offer['id']}")
        self.assertTrue(status.startswith("200"))
        self.assertEqual(deleted_offer["deleted_offer_id"], offer["id"])

        status, deleted_product = self._request("DELETE", f"/api/products/{product['id']}")
        self.assertTrue(status.startswith("200"))
        self.assertEqual(deleted_product["deleted_product_id"], product["id"])

    def test_staged_patch_delete_and_alert_deactivate(self) -> None:
        _, staged = self._request(
            "POST",
            "/api/admin/staged/products",
            {"title": "Nikon Z6", "brand": "Nikon", "category": "camera", "condition": "used", "source": "manual"},
        )

        status, staged_updated = self._request(
            "PATCH",
            f"/api/admin/staged/products/{staged['id']}",
            {"title": "Nikon Z6 II", "brand": "Nikon", "category": "camera", "condition": "used"},
        )
        self.assertTrue(status.startswith("200"))
        self.assertEqual(staged_updated["title"], "Nikon Z6 II")

        _, staged_offer = self._request(
            "POST",
            "/api/admin/staged/offers",
            {
                "staged_product_id": staged["id"],
                "seller_name": "PhotoCity",
                "seller_city": "Baku",
                "price_azn": 2500,
                "currency": "AZN",
            },
        ...
        status, staged_offer_updated = self._request(
            "PATCH",
            f"/api/admin/staged/offers/{staged_offer['id']}",
            {
                "seller_name": "PhotoCity Pro",
                "seller_city": "Baku",
                "price_azn": 2450,
                "currency": "AZN",
                "is_available": True,
        ...
        self.assertTrue(status.startswith("200"))
        self.assertEqual(staged_offer_updated["seller_name"], "PhotoCity Pro")

        status, del_offer = self._request("DELETE", f"/api/admin/staged/offers/{staged_offer['id']}")
        self.assertTrue(status.startswith("200"))
        self.assertEqual(del_offer["deleted_staged_offer_id"], staged_offer["id"])

        status, del_product = self._request("DELETE", f"/api/admin/staged/products/{staged['id']}")
        self.assertTrue(status.startswith("200"))
        self.assertEqual(del_product["deleted_staged_product_id"], staged["id"])

        _, product = self._request("POST", "/api/products", {
            "title": "MacBook Air", "brand": "Apple", "category": "laptops", "condition": "new"
        })
        _, alert = self._request("POST", "/api/alerts", {
            "product_id": product["id"], "target_price_azn": 1800, "contact_email": "notify@example.com"
        })
        status, deactivated = self._request("POST", f"/api/alerts/{alert['id']}/deactivate")
        self.assertTrue(status.startswith("200"))
