...
from apps.api.repository import (
    ...
    delete_staged_offer,
    delete_staged_product,
    list_seller_reviews,
    publish_staged_product,
    update_product,
    update_staged_offer,
    update_staged_product,
...
def _parse_int_id(raw: str, field: str) -> tuple[int | None, str | None]:
    try:
        value = int(raw)
    except ValueError:
...
def application(environ, start_response):
    ...
    if path.startswith("/api/admin/staged/products/") and method == "PATCH":
        staged_id_raw = path.rsplit("/", 1)[-1]
        staged_id, err = _parse_int_id(staged_id_raw, "staged_product_id")
        if err:
    ...
    if path.startswith("/api/admin/staged/products/") and method == "DELETE":
        staged_id_raw = path.rsplit("/", 1)[-1]
        staged_id, err = _parse_int_id(staged_id_raw, "staged_product_id")
        if err:
            return json_response(start_response, "400 Bad Request", {"error": err})
        try:
            return json_response(start_response, "200 OK", delete_staged_product(staged_id))
        except ValueError as exc:
            return json_response(start_response, "404 Not Found", {"error": str(exc)})

    if path.startswith("/api/admin/staged/offers/") and method == "PATCH":
        staged_offer_id_raw = path.rsplit("/", 1)[-1]
        staged_offer_id, err = _parse_int_id(staged_offer_id_raw, "staged_offer_id")
        if err:
            return json_response(start_response, "400 Bad Request", {"error": err})
        data = parse_json_body(environ)
        if not _is_valid_text(data.get("seller_name"), min_len=2, max_len=120):
    ...
    if path.startswith("/api/admin/staged/offers/") and method == "DELETE":
        staged_offer_id_raw = path.rsplit("/", 1)[-1]
        staged_offer_id, err = _parse_int_id(staged_offer_id_raw, "staged_offer_id")
        if err:
            return json_response(start_response, "400 Bad Request", {"error": err})
        try:
            return json_response(start_response, "200 OK", delete_staged_offer(staged_offer_id))
        except ValueError as exc:
            return json_response(start_response, "404 Not Found", {"error": str(exc)})
    ...
    if path.startswith("/api/products/") and method == "GET" and not path.endswith("/card"):
        product_id_raw = path.rsplit("/", 1)[-1]
        product_id, err = _parse_int_id(product_id_raw, "product_id")
        if err:
            return json_response(start_response, "400 Bad Request", {"error": err})
        try:
            return json_response(start_response, "200 OK", get_product(product_id))
        except ValueError as exc:
            return json_response(start_response, "404 Not Found", {"error": str(exc)})
    ...
    if path.startswith("/api/products/") and method == "DELETE":
        ...
        if err:
            return json_response(start_response, "400 Bad Request", {"error": err})
        try:
            return json_response(start_response, "200 OK", delete_product(product_id))
        except ValueError as exc:
            return json_response(start_response, "404 Not Found", {"error": str(exc)})
    ...
    if path.startswith("/api/offers/") and method == "DELETE":
        offer_id_raw = path.rsplit("/", 1)[-1]
        offer_id, err = _parse_int_id(offer_id_raw, "offer_id")
        if err:
            return json_response(start_response, "400 Bad Request", {"error": err})
        try:
            return json_response(start_response, "200 OK", delete_offer(offer_id))
        except ValueError as exc:
            return json_response(start_response, "404 Not Found", {"error": str(exc)})
    ...
    if path.startswith("/api/alerts/") and path.endswith("/deactivate") and method == "POST":
        alert_id_raw = path.split("/")[3]
        alert_id, err = _parse_int_id(alert_id_raw, "alert_id")
        if err:
            return json_response(start_response, "400 Bad Request", {"error": err})
        try:
            return json_response(start_response, "200 OK", deactivate_price_alert(alert_id))
        except ValueError as exc:
            return json_response(start_response, "404 Not Found", {"error": str(exc)})
